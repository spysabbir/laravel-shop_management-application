        ...
        <div class="card">
                    ...
                    <div class="modal fade" id="trashedModal" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <table class="table table-primary" id="trashed_expense_category_table" style="width: 100%">
                                        <thead>
                                            <tr>
                                                <th>Sl No</th>
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-light" id="all_expense_category_table">
                        <thead>
                            <tr>
                                <th>Sl No</th>
                                <th>Expense Category Name</th>
                                <th>Expense Category Status</th>
                                <th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form action="#" method="POST" id="edit_form">
                                                @csrf
                                                @method('PATCH')
                                                <input type="hidden" name="" id="expense_category_id">
                                                <div class="mb-3">
                                                    <label class="form-label">Expense Category Name</label>
                                                    <input type="text" name="expense_category_name" class="form-control" id="expense_category_name"/>
                                                    <span class="text-danger error-text update_expense_category_name_error"></span>
                                                </div>
...
<script>
    $(document).ready(function() {
        ...
        // Read Data
        table = $('#all_expense_category_table').DataTable({
            processing: true,
            serverSide: true,
            searching: true,
            ajax: {
        ...
        $(document).on('change', '.filter_data', function(e){
            e.preventDefault();
            $('#all_expense_category_table').DataTable().ajax.reload()
        })
        ...
        $(document).on('click', '.editBtn', function(e){
            e.preventDefault();
            var id = $(this).attr('id');
            var url = "{{ route('expense-category.edit', ":id") }}";
            url = url.replace(':id', id)
            $.ajax({
                url:  url,
                method: 'GET',
                success: function(response) {
                    $("#expense_category_name").val(response.expense_category_name);
        ...
        $('#edit_form').on('submit', function(e){
            ...
            var url = "{{ route('expense-category.update', ":id") }}";
            url = url.replace(':id', id)
            const form_data = new FormData(this);
        ...
        $(document).on('click', '.deleteBtn', function(e){
            e.preventDefault();
            let id = $(this).attr('id');
            var url = "{{ route('expense-category.destroy', ":id") }}";
            url = url.replace(':id', id)
            Swal.fire({
                title: 'Are you sure?',
        ...
        // Trashed Data
        trashed_table = $('#trashed_expense_category_table').DataTable({
            processing: true,
            serverSide: true,
            searching: true,
            ajax: {
                url: "{{ route('expense-category.trashed') }}",
            },
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'expense_category_name', name: 'expense_category_name'},
        ...
        $(document).on('click', '.restoreBtn', function(e){
            e.preventDefault();
            let id = $(this).attr('id');
            var url = "{{ route('expense-category.restore', ":id") }}";
            url = url.replace(':id', id)
            $.ajax({
                url: url,
                method: 'GET',
        ...
        $(document).on('click', '.forceDeleteBtn', function(e){
            ...
            $('.btn-close').trigger('click');
            let id = $(this).attr('id');
            var url = "{{ route('expense-category.forcedelete', ":id") }}";
            url = url.replace(':id', id)
            Swal.fire({
                title: 'Are you sure?',
                text: "You won't be able to revert this!",
        ...
        $(document).on('click', '.statusBtn', function(e){
            e.preventDefault();
            let id = $(this).attr('id');
            var url = "{{ route('expense-category.status', ":id") }}";
            url = url.replace(':id', id)
            $.ajax({
                url: url,
                method: 'GET',
