	...
	<div class="container">
		<header>
			...
			<div class="invoice-info">
				<h1>Invoice: #{{ $purchase_summary->purchase_invoice_no }}</h1>
				<p>Date: {{ $purchase_summary->purchase_date }}</p>
			</div>
		</header>
		<section class="details">
			<div class="customer-info">
				<h3>Customer </h3>
				<p>{{ $purchase_summary->relationtosupplier->supplier_name }}<br>{{ $purchase_summary->relationtosupplier->supplier_address }}</p>
				<p>{{ $purchase_summary->relationtosupplier->supplier_phone_number }}<br>{{ $purchase_summary->relationtosupplier->supplier_email }}</p>
			</div>
		...
		<section class="invoice-table">
			<table>
				...
				<tbody>
					...
					<tr>
						<td>{{ $purchase_detail->relationtoproduct->product_name }}</td>
						<td>{{ $purchase_detail->purchase_quantity }}</td>
						<td>{{ $purchase_detail->purchase_price }}</td>
						<td>{{ $purchase_detail->purchase_quantity * $purchase_detail->purchase_price }}</td>
					</tr>
                    @endforeach
				</tbody>
				<tfoot>
